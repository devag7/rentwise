 ...
 * To populate initial data, run: npx tsx scripts/apify_sync.ts
 */

import cron from 'node-cron';
import { runScraper } from './scraper.js';
import { runApifySync } from './apify_sync.js';

console.log('[CRON] RentWise Cron Scheduler initialized');
console.log('[CRON] Scrapers scheduled at 06:00 and 18:00 IST daily');
...
cron.schedule('0 6 * * *', async () => {
    ...
    try {
        ...
        console.error('[CRON] Morning MB scrape failed:', err);
    }
    try {
        await runApifySync();
    } catch (err) {
        console.error('[CRON] Morning Apify scrape failed:', err);
    }
}, {
...
cron.schedule('0 18 * * *', async () => {
    ...
    try {
        ...
        console.error('[CRON] Evening MB scrape failed:', err);
    }
    try {
        await runApifySync();
    } catch (err) {
        console.error('[CRON] Evening Apify scrape failed:', err);
    }
}, {
    timezone: 'Asia/Kolkata'
});
